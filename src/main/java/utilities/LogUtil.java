...
public class LogUtil {
	 ...
	 * @param clazz
	 */
	public synchronized void init(Class clazz) {
		if (!isInit) {
			try {
				htmlLogger = Logger.getLogger(clazz + ",HtmlLogger");
				htmlLogger.setLevel(Level.INFO);
				htmlLayout.setTitle("Automation Logs");
				htmlFileApp = new FileAppender(htmlLayout, config.getValue("logHtmlFilePath"));
			...
			normalFileApp.setFile(config.getValue("logInfoFilePath"));
			...
			try {
				...
				errorLog(LogUtil.class, "Exception caught", e);
			}
			...
			errorFileApp.setFile(config.getValue("logErrorFilePath"));
	 ...
	 * @param className
	 */
	public synchronized void init(String className) {
		if (!isInit) {
			try {
	 ...
	 * @param message
	 */
	public synchronized void infoLog(Class clazz, String message) {
		init(clazz);
	 ...
	 * @param message
	 */
	public synchronized void infoLog(String className, String message) {
		init(className);
		normalLogger.info(message);
		htmlLogger.info(message);
	}
	 ...
	 * @param t
	 */
	public synchronized void errorLog(Class clazz, String message, Throwable t) {
		init(clazz);
	 ...
	 * @param message
	 */
	public synchronized void errorLog(Class clazz, String message) {
		init(clazz);
	 ...
	 * @param message
	 */
	public synchronized void errorLog(String name, String message) {
		init(name);
		htmlLogger.error(message);
		errorLogger.error(message);
		errorLogger.error("-----------------------------------------------------------------------");
